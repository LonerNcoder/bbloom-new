...
function extract(xml) {
  ...
  import {verifyUser, classifyGenre} from "~/server/utils";
  ...
  export default defineEventHandler(async (event) => {
    ...
    if (!isVerified){
        ...
        try {
          ...
          const id = await processEpub(filePath, uniqueDirPath, event.context.session.user?.id);
  ...
  // async function processEpub(filePath, uniqueDirPath) {
  //   const directory = await unzipper.Open.file(filePath);
  
  //   // Unzip files into the unique directory
  //   await Promise.all(
  ...
  //   // Extract CSS files, OPF file, XHTML files, and images
  ...
  //   // const xhtmlFiles = directory.files.filter((file) => file.path.endsWith('.xhtml'));
  //   const imageExtensions = /\.(svg|png|jpeg|jpg|webp|gif|bmp|tiff)$/i;
  //   const imageFiles = directory.files.filter((file) => imageExtensions.test(file.path));
  
  //   // Extract full paths of images
  //   const imagePaths = imageFiles.map((file) => path.join(uniqueDirPath, file.path));
  //   console.log(`Found ${imagePaths.length} image(s) in the EPUB.`);
  //   // Identify and upload the cover image
  //   const coverImage = imagePaths.find((imagePath) => /\\cover\.[a-z0-9]+$/i.test(imagePath));
  //   var coverImageUrl = null;
  
  //   if (coverImage) {
  //     // console.log(`Found cover image: ${coverImage}`);
  //     coverImageUrl = await uploadImage(coverImage, path.basename(coverImage));
  //     console.log(`Cover image uploaded with URL: ${coverImageUrl}`);
  //   } else {
  //     console.warn('No cover image found in the EPUB.');
  //   }
  
  
  //   // Proceed with existing logic for CSS, OPF, and chapter processing
  ...
  //   // Further process EPUB content as in the previous implementation
  ...
  //       await Promise.all(chapters.map(async (chapterId, index) => {
  ...
  //               console.log(`Relative image src: ${relativeSrc}`);
  //               const resolvedSrcPath = path.resolve(chapterBasePath, relativeSrc);
  //               console.log(`Resolved image path: ${resolvedSrcPath}`);
  ...
  //             // Save the chapter with updated HTML
  ...
  async function processEpub(filePath, uniqueDirPath, userId) {
    ...
    const genres = classifyGenre(subjects);
        ...
        try {
          ...
          const newNovel = await prisma.novel.create({
            data: {
              ...
              tags: {
                connectOrCreate: genres.map((tag) => ({
                  where: { name: tag },
                  create: { name: tag },
                })),
              },
          ...
          novelId = newNovel.id;
          const reviewInstanceId = `Novel_${novelId}`;
